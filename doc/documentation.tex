...
The model is defined automatically when `act-up.lisp' is loaded.  To reset the model, use the `reset-model' function.  To create a new model (multiple models may be used in parallel), use `(make-model)'.  Set the Lisp variable *current-actUP-model* to define the current model.
