...
;;; Copyright   : (c) 2004-5 Dan Bothell/Mike Byrne
...
(defun posnum (x)
  "Returns T only if <x> is a positive number"
  (and (numberp x) (plusp x)))

(defun nonneg (x)
  "Returns T only if <x> is a non-negative number"
  (and (numberp x) (>= x 0.)))


(defun numornil (x)
  "Returns T only if <x> is a number or nil"
  (or (null x) (numberp x)))

(defun posnumornil (x)
  "Returns T only if <x> is a positive number or nil"
  (or (null x) (posnum x)))

(defun nonnegornil (x)
  "Returns T only if <x> is a non-negative number or nil"
  (or (null x) (nonneg x)))

(defun numorbool (x)
  "Returns t only if <x> is a number, T or nil"
